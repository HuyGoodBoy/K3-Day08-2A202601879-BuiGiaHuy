...
# LLM model — dùng OpenRouter (OpenAI-compatible API)
# Gợi ý model miễn phí: openai/gpt-4o-mini (free tier)
...
def format_context(chunks: list[dict]) -> str:
    ...
    """
    if not chunks:
        return "Không có tài liệu liên quan trong cơ sở dữ liệu."
...
def generate_with_citation(query: str, top_k: int = TOP_K) -> dict:
    ...
    if not chunks:
        return {
            "answer": "Tôi không tìm thấy thông tin liên quan trong cơ sở dữ liệu. "
                      "Hãy thử hỏi câu khác hoặc liên hệ bộ phận hỗ trợ sinh viên.",
            "sources": [],
    ...
    if not api_key:
        # Fallback: trả lời đơn giản không cần LLM
        top_source = chunks[0].get("metadata", {}).get("source", "Unknown")
        answer = (
            f"Tôi tìm thấy thông tin liên quan từ [{top_source}]. "
            f"Nội dung chính: {chunks[0]['content'][:300]}...\n\n"
            f"(Cần OPENROUTER_API_KEY trong .env để sinh câu trả lời tự động)"
        )
        return {
            "answer": answer,
            "sources": chunks,
    ...
    try:
        ...
        # LLM call failed — return retrieval result as fallback
        answer = (
            f"Lỗi gọi LLM: {e}\n\n"
            f"Tài liệu liên quan nhất:\n"
            + "\n\n".join(
